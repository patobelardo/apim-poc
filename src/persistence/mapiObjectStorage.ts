...
import { TenantMapiClient } from "../services/tenantMapiClient";
...
export class MapiObjectStorage implements IObjectStorage {
    constructor(private readonly tenantMapiClient: TenantMapiClient) { }
    ...
    public async addObject<T>(path: string, dataObject: T): Promise<void> {
        ...
        try {
            const headers: HttpHeader[] = [TenantMapiClient.getPortalHeader("addObject")];
            await this.tenantMapiClient.put<T>(resource, headers, { properties: converted });
        }
        catch (error) {
    ...
    public async getObject<T>(key: string): Promise<T> {
        try {
            ...
            const item = await this.tenantMapiClient.get<T>(`${resource}`);
    ...
    public async deleteObject(path: string): Promise<void> {
        ...
        try {
            ...
            headers.push({ name: "If-Match", value: "*" }, TenantMapiClient.getPortalHeader("deleteObject"));

            await this.tenantMapiClient.delete(resource, headers);
        }
        catch (error) {
    ...
    public async updateObject<T>(key: string, dataObject: T): Promise<void> {
        ...
        try {
            ...
            await this.tenantMapiClient.head<T>(resource);
            exists = true;
        }
        catch (error) {
            if (error?.code === "ResourceNotFound") {
        ...
        try {
            const headers: HttpHeader[] = [TenantMapiClient.getPortalHeader("updateObject")];

            if (exists) {
                headers.push({ name: "If-Match", value: "*" });
            }

            await this.tenantMapiClient.put<T>(resource, headers, { properties: armContract });
        }
        catch (error) {
            throw new AppError(`Could not update object '${key}'.`, error);
        }
    ...
    private async loadNextPage<T>(resource: string, localeSearchPrefix: string, filterQueryString: string, skip: number, isLocalized: boolean): Promise<Page<T>> {
        ...
        const pageOfTs = await this.tenantMapiClient.get<PageContract<T>>(url);
        const searchResult = [];
    ...
    public async searchObjects<T>(key: string, query: Query<T>): Promise<Page<T>> {
        ...
        try {
            ...
            if (key.includes("navigationItems")) {
                const armContract = await this.tenantMapiClient.get<any>(`${resource}?$orderby=${localeSearchPrefix}title${filterQueryString}`);
