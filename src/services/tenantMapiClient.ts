...
export class TenantMapiClient {
    ...
    constructor(
        private readonly tenantHttpClient: HttpClient,
        private readonly tenantAuthenticator: IAuthenticator,
    ...
    private async initialize(): Promise<void> {
        ...
        const managementApiUrl = settings[Constants.SettingNames.managementApiUrl + "Tenant"];

        if (!managementApiUrl) {
            throw new Error(`Management API URL ("${Constants.SettingNames.managementApiUrl + "Tenant"}") setting is missing in configuration file.`);
        }

        this.managementApiUrl = Utils.ensureUrlArmified(managementApiUrl);

        const managementApiAccessToken = settings[Constants.SettingNames.managementApiAccessToken + "Tenant"];

        if (managementApiAccessToken) {
            const accessToken = AccessToken.parse(managementApiAccessToken);
            await this.tenantAuthenticator.setAccessToken(accessToken);
        }
        else if (this.environment === "development") {
            console.warn(`Development mode: Please specify ${Constants.SettingNames.managementApiAccessToken + "Tenant"}" in configuration file.`);
            return;
        }
    ...
    protected async makeRequest<T>(httpRequest: HttpRequest): Promise<T> {
        ...
        if (!authHeader || !authHeader.value) {
            const authToken = await this.tenantAuthenticator.getAccessToken();
        ...
        if (!portalHeader) {
            httpRequest.headers.push(TenantMapiClient.getPortalHeader());
        }
        ...
        try {
            response = await this.tenantHttpClient.send<T>(httpRequest);
        }
        catch (error) {
        ...
        try {
            await this.tenantAuthenticator.refreshAccessTokenFromHeader(response.headers);
        }
        catch (error) {
    ...
    private createMapiError(statusCode: number, url: string, getError: () => any): any {
        switch (statusCode) {
            ...
            case 401:
                this.tenantAuthenticator.clearAccessToken();
